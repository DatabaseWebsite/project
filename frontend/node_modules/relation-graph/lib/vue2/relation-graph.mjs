...
var ct = (e, i, t) => i in e ? se(e, i, { enumerable: !0, configurable: !0, writable: !0, value: t }) : e[i] = t, wt = (e, i) => {
  for (var t in i || (i = {}))
    le.call(i, t) && ct(e, t, i[t]);
  if (xt)
    for (var t of xt(i))
...
(function(e) {
  ...
  i = function() {
    var d, f, h, _;
    (_ = document.createElement("div")).innerHTML = l, l = null, (h = _.getElementsByTagName("svg")[0]) && (h.setAttribute("aria-hidden", "true"), h.style.position = "absolute", h.style.width = 0, h.style.height = 0, h.style.overflow = "hidden", d = h, (f = document.body).firstChild ? (_ = d, (h = f.firstChild).parentNode.insertBefore(_, h)) : f.appendChild(d));
...
const he = (e) => (e = e.replace("#", ""), e = e.replace("(", ""), e = e.replace(")", ""), e = e.replace(/,/, "-"), e), U = (e) => {
  try {
...
}, ft = (e, i, t) => e ? e.classList.contains(i) ? e : e.classList.contains(t) ? null : ft(e.parentElement, i, t) : null, de = (e) => w(void 0, null, function* () {
    ...
    i.forEach((r) => {
      ...
      if (n === 0) {
        ...
        r.targetNodes.forEach((f) => {
          f.lot || (f.lot = { eached: !1, childs: [] }), f.lot.eached || (g.isAllowShowNode(f) ? (f.lot.eached = !0, f.lot.parent = r, f.lot.index_of_parent = d++, r.lot.childs.push(f), a.push(f), c++) : r.lot.childs.push(f));
        });
      } else if (n === -1) {
        let d = 0;
  ...
  isRectangleOverlap(e, i) {
    const t = e.x + $, o = i.x + $, n = e.el.offsetWidth - $ * 2, s = i.el.offsetWidth - $ * 2, a = e.y + $, l = i.y + $, r = e.el.offsetHeight - $ * 2, c = i.el.offsetHeight - $ * 2;
  ...
  getNoOverlapLimitedPosition(e, i, t, o) {
    const n = e.x + $, s = e.y + $, a = i + $, l = o.x + $, r = e.el.offsetWidth - $ * 2, c = o.el.offsetWidth - $ * 2, d = t + $, f = o.y + $, h = e.el.offsetHeight - $ * 2, _ = o.el.offsetHeight - $ * 2;
    let p = e.x, m = e.y;
    const y = this.isXOverlap(n, l, r, c), P = this.isYOverlap(s, f, h, _);
    return y ? d < f ? (p = i, m = f - h - $) : d > f && (p = i, m = f + _ - $) : P && (a < l ? (p = l - r - $, m = t) : a > l && (p = l + c - $, m = t)), { x: p, y: m };
  }
}, $ = 8, ye = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  RGNodesAnalytic: g,
  default: g
  ...
  playAnimation(i) {
    ...
    this.allNodes.forEach((t) => {
      ...
      if (o !== void 0 && n !== void 0 && s !== void 0 && a !== void 0) {
        const l = (s - o) / this.allAnimationStep, r = (a - n) / this.allAnimationStep, c = Math.floor(o + this.currentAnimationStep * l), d = Math.floor(n + this.currentAnimationStep * r);
        (t.x !== c || t.y !== d) && (t.x = c, t.y = d);
      }
...
class Vt extends it {
  ...
  analysisNodes4Didirectional(t, o, n, s, a) {
    ...
    o.forEach((d) => {
      ...
      }), d.lot.strength = f > 0 ? f : 1, l.all_strength += d.lot.strength, d.lot.strength_plus = l.all_strength, d.lot.index_of_level = c, d.lot.childs_size = f, c++;
  ...
  placeRelativePosition(t, o, n) {
    ...
    if (this.layoutOptions.from === "left" || this.layoutOptions.from === "right") {
      const a = R(this.layoutOptions.min_per_height) || 80, l = R(this.layoutOptions.max_per_height) || 400, r = R(this.layoutOptions.min_per_width) || 430, c = R(this.layoutOptions.max_per_width) || 650;
      ...
      f < a && (f = a), f > l && (f = l), o.forEach((h) => {
        ...
        h.fixed !== !0 && h.lot.level !== 0 && (h.lot.y = t.lot.y + f * (n.max_strength / -2 + h.lot.strengthWithChilds_from + h.lot.strengthWithChilds / 2));
      }), this.gatherNodes(o, "h", f);
    } else {
      const a = R(this.layoutOptions.min_per_height) || 350, l = R(this.layoutOptions.max_per_height) || 400, r = R(this.layoutOptions.min_per_width) || 250, c = R(this.layoutOptions.max_per_width) || 500;
      ...
      f < a && (f = a), f > l && (f = l), o.forEach((h) => {
        ...
        h.fixed !== !0 && h.lot.level !== 0 && (h.lot.x = t.lot.x + d * (n.max_strength / -2 + h.lot.strengthWithChilds_from + h.lot.strengthWithChilds / 2));
...
function ve(e, i, t, o, n, s, a, l, r) {
  const c = n - e, d = s - i, f = Math.sqrt(c * c + d * d), h = r * d / f, _ = -(r * c) / f, p = e + h, m = i + _, y = e - t / 2, P = e + t / 2, N = i - o / 2, L = i + o / 2;
  let M = [];
  if (c !== 0) {
    let O = (y - p) / c, x = m + O * d;
    x >= N && x <= L && M.push({ x: y, y: x });
    let S = (P - p) / c, G = m + S * d;
    G >= N && G <= L && M.push({ x: P, y: G });
  }
  if (d !== 0) {
    let O = (N - m) / d, x = p + O * c;
    x >= y && x <= P && M.push({ x, y: N });
    let S = (L - m) / d, G = p + S * c;
    G >= y && G <= P && M.push({ x: G, y: L });
  }
  let z = null, E = 1 / 0;
  return M.forEach((O) => {
    const x = Math.sqrt((O.x - n) ** 2 + (O.y - s) ** 2);
    x < E && (E = x, z = O);
  }), z;
}
const I = {
  getRectPoint(e, i, t, o, n, s, a, l, r = !1, c = 1, d = 1) {
    r && (d = c - d - 1);
    const f = d - (c - 1) / 2, h = e + n / 2, _ = i + s / 2, p = t + a / 2, m = o + l / 2;
    if (c > 1) {
      const x = 14 * f, S = ve(
        h,
        _,
    ...
    const y = h < p ? 1 : -1, P = _ < m ? 1 : -1;
    if (m === _)
      return { x: h + y * n / 2, y: _ };
    const N = Math.abs((p - h) / (m - _)), L = n / s;
    let M = 0, z = 0;
    N < L ? (M = y * s / 2 * N + P * L, z = P * s / 2) : (M = y * n / 2, z = P * n / 2 / N + y * L);
    const E = h + M, O = _ + z;
    return { x: E, y: O };
  },
  getRectPointBasic(e, i, t, o, n, s, a, l) {
    const r = e + n / 2, c = i + s / 2, d = t + a / 2, f = o + l / 2;
    let h = f === c ? 0 : (d - r) / (f - c);
    h === 0 && (h = (d - r) / (f - c + 1));
    const _ = n / l;
    let p = 0, m = 0, y = "1";
    return -1 * _ < h && h < _ ? (y = "2", c < f ? (p = s / 2 * h, m = s / 2) : (p = -1 * s / 2 * h, m = -1 * s / 2)) : (r < d ? (p = 1 * n / 2, m = 1 * n / 2 / h) : (p = -1 * n / 2, m = -1 * n / 2 / h), y = "3"), { x: r + p, y: c + m, _case: y };
  },
  getRectJoinPoint(e, i, t, o, n, s, a, l) {
    const r = e + n / 2, c = i + s / 2, d = t + a / 2, f = o + l / 2, h = Math.round(Math.atan2(f - c, d - r) * 180 / 3.14) + 135;
  ...
  getBorderPoint4MultiLine(e, i, t, o, n, s, a, l, r, c, d, f) {
    return r === 0 ? this.getCirclePoint4MultiLine(e, i, t, o, n, s, a, l, c, d, f) : this.getRectPoint(e, i, t, o, n, s, a, l, c, d, f);
  },
  getCirclePoint(e, i, t, o, n, s, a, l) {
    const r = t + a / 2, c = o + l / 2, d = e + n / 2, f = i + s / 2, h = r - d;
    if (h === 0)
      return { x: d, y: f - s / 2 * (c < f ? 1 : -1) };
    const p = (c - f) / h, m = Math.sqrt(1 / (1 / (n / 2) ** 2 + p ** 2 / (s / 2) ** 2)) * (r < d ? 1 : -1), y = p * m;
    return { x: d - m, y: f - y };
  },
  getCirclePoint4MultiLine(e, i, t, o, n, s, a, l, r, c, d) {
    r && (d = c - d - 1);
    const f = t + a / 2, h = o + l / 2, _ = e + n / 2, p = i + s / 2, m = f - _;
    if (m === 0)
      return { x: _, y: p - s / 2 * (h < p ? 1 : -1) };
    const P = (d - (c - 1) / 2) * 14, N = h - p, L = Math.sqrt(m ** 2 + N ** 2) * P / m, M = N / m, z = n / 2, E = s / 2, O = f < _ ? 1 : -1, x = (-1 * z ** 2 * M * L + z * E * Math.sqrt(E ** 2 + M ** 2 * z ** 2 - L ** 2) / O) / (E ** 2 + z ** 2 * M ** 2), S = M * x + L;
    return { x: _ - x, y: p - S };
  },
  getCirclePointBasic(e, i, t, o, n, s, a, l, r) {
    const c = e + n / 2, d = i + s / 2, f = t + a / 2, h = o + l / 2, _ = f - c, p = h - d, m = Math.sqrt(_ * _ + p * p), y = m - r, P = y * _ / m * -1, N = y * p / m * -1;
    return { x: f + P, y: h + N };
  },
  getCirclePointPlus(e, i, t, o, n, s, a, l) {
    const r = e + n / 2, c = i + s / 2, d = t + a / 2, f = o + l / 2, h = d - r, _ = f - c, p = Math.sqrt(h * h + _ * _), m = (p - n / 2) * h / p * -1, y = (p - s / 2) * _ / p * -1;
    return { x: d + m, y: f + y };
  },
  getOvalPoint(e, i, t, o, n) {
    return {
      x: e + t * Math.sin((Lt + o * (360 / n) + 0) * Math.PI / 180),
      y: i + t * Math.cos((Lt + o * (360 / n) + 0) * Math.PI / 180) * -1
    };
  },
  getAngleType(e, i) {
    if (e >= 0 && i >= 0)
...
class et extends it {
  ...
  easyPlaceRelativePosition(t, o) {
    o.forEach((s) => {
      if (s.lot.subling.level === 1) {
        ...
        if (a) {
          const l = I.getOvalPoint(t.lot.x, t.lot.y, a.level * 50, s.lot.strength_plus - s.lot.strength / 2, a.all_strength);
      ...
      const a = s.lot.subling;
      if (a && a.level > 1) {
        const l = s.lot.parent.lot.strength_plus - s.lot.parent.lot.strength, c = (s.lot.parent.lot.strength_plus - l) / (s.lot.parent.lot.childs.length + 1) * (s.lot.index_of_parent + 1), d = I.getOvalPoint(t.lot.x, t.lot.y, (a.level - 1) * 40 + 50, l + c, s.lot.parent.lot.subling.all_strength);
  ...
  doForceLayout(t) {
    ...
    if (u("this.layoutTimes:", this.layoutTimes, "of", this.maxLayoutTimes, "Current refresh rate:", o, "Hz", this.requireLinks, this.allLinks.length), this.layoutTimes > this.maxLayoutTimes) {
  ...
  addElasticByLine(t, o, n = 1) {
    ...
    const a = s < this.minVaildLineLength ? 0 : (s - this.minVaildLineLength) * 0.1 * this.force_line_elastic * n, l = a, r = a, c = (this.getX(t) - this.getX(o)) / s, d = (this.getY(t) - this.getY(o)) / s;
    this.addFtoNode(t, c * l * -1, d * l * -1), this.addFtoNode(o, c * r, d * r);
  }
  ...
  applyToNodePosition(t) {
    ...
    o > this.maxMoveSpeed && (o = this.maxMoveSpeed), n > this.maxMoveSpeed && (n = this.maxMoveSpeed), o < -this.maxMoveSpeed && (o = -this.maxMoveSpeed), n < -this.maxMoveSpeed && (n = -this.maxMoveSpeed), t.x = t.x + o, t.y = t.y + n, t.Fx = 0, t.Fy = 0;
  }
}
class jt extends et {
  constructor(t, o) {
  ...
  placeNodes(t, o) {
    return w(this, null, function* () {
      ...
      if (g.analysisNodes(n, [this.rootNode], 0, s), n = [], s = {
        ...
        o.origin_x === void 0 && (o.origin_x = o.x, o.origin_y = o.y), o.lot.x = o.origin_x, o.lot.y = o.origin_y, u("\u56FA\u5B9A\u4F4D\u7F6E\u7684rootNode:", o.text, o.x, o.y);
      ...
      u("root position:", o.lot.x, o.lot.y), this.placeRelativePosition(this.rootNode, n, s), this.graphOptions.useAnimationWhenExpanded ? (u("Play layout animation....."), yield this.animationLayout()) : n.forEach((a) => {
        if (a.fixed === !0 || !g.isAllowShowNode(a))
          return;
        const l = a.offset_x || 0, r = a.offset_y || 0, c = -g.getNodeWidth(a, this.graphOptions) / 2, d = -g.getNodeHeight(a, this.graphOptions) / 2;
        a.x = a.lot.x + l + c, a.y = a.lot.y + r + d, a.lot.placed = !0;
      });
    });
  }
  placeRelativePosition(t, o, n) {
    ...
    u("analyticResult:", n, a, this.layoutOptions.distance_coefficient), a < 150 * s && (a = 150 * s);
    let l = 0;
    o.forEach((c) => {
      const d = c.lot.subling;
      if (d && d.level === 1) {
        l = Math.round(d.all_size * 50 / Math.PI / 2), l < a && (l = a);
        const f = I.getOvalPoint(t.lot.x, t.lot.y, d.level * l, c.lot.strength_plus - c.lot.strength / 2, d.all_strength);
    ...
    o.forEach((c) => {
      const d = c.lot.subling;
      if (d && d.level > 1) {
        const f = c.lot.parent.lot.strength_plus - c.lot.parent.lot.strength, _ = (c.lot.parent.lot.strength_plus - f) / (c.lot.parent.lot.childs_size + 1) * (c.lot.index_of_parent + 1), p = I.getOvalPoint(t.lot.x, t.lot.y, (d.level - 1) * r + l, f + _, c.lot.parent.lot.subling.all_strength);
  ...
  placeNodes(i, t) {
    return w(this, null, function* () {
      ...
      l < 200 && (l = 200), l > 800 && (l = 800), this.allNodes.forEach((r, c) => {
        const d = I.getOvalPoint(s.x, s.y, l, c, a);
        ...
        if (r.fixed === !0 || !g.isAllowShowNode(r))
          return;
        const c = r.offset_x || 0, d = r.offset_y || 0, f = -g.getNodeWidth(r, this.graphOptions) / 2, h = -g.getNodeHeight(r, this.graphOptions) / 2;
        r.x = r.lot.x + c + f, r.y = r.lot.y + d + h, r.lot.placed = !0;
      });
    });
  }
}
...
class xe extends it {
  ...
  analysisNodes4Didirectional(t, o, n, s) {
    ...
    t.forEach((c) => {
      const d = c.lot.level + "";
      a[d] || (a[d] = []), a[d].push(c), c.lot.subling = {
        level: c.lot.level,
        all_size: 0,
        all_strength: 0
      };
    }), t.forEach((c) => {
      const d = c.lot.level + "", f = a[d], h = f.length;
    ...
    }), console.log("willLayoutNodes.length:", t.length), t.forEach((c) => {
      c.lot.childs_size === 0 && (c.lot.strengthWithChilds = 1, g.conductStrengthToParents(c));
    });
    const l = Object.keys(a).map((c) => Number.parseInt(c)).sort(), r = (c) => {
      const d = c + "", f = a[d];
      let h = 0;
      f.forEach((_) => {
        _.lot.parent && h < _.lot.parent.lot.strengthWithChilds_from && (h = _.lot.parent.lot.strengthWithChilds_from), c === 0 ? h += 1 : _.lot.strengthWithChilds_from = h, u("levels:", c, h, _.text, _.lot.strengthWithChilds), h += _.lot.strengthWithChilds;
      });
    };
  ...
  analysisBothWay(t, o, n, s) {
    ...
    o.forEach((r) => {
      ...
      r.targetFrom.forEach((h) => {
        h.lot || (h.lot = { eached: !1, childs: [] }), h.lot.eached || (h.lot.parent = r, g.isAllowShowNode(h) ? (h.lot.eached = !0, h.lot.index_of_parent = d++, r.lot.childs.push(h), f.push(h), c++) : r.lot.childs.push(h));
      }), this.analysisBothWay(t, f, n - 1, s), f = [], r.targetTo.forEach((h) => {
        h.lot || (h.lot = { eached: !1, childs: [] }), h.lot.eached || (h.lot.parent = r, g.isAllowShowNode(h) ? (h.lot.eached = !0, h.lot.index_of_parent = d++, r.lot.childs.push(h), f.push(h), c++) : r.lot.childs.push(h));
      }), this.analysisBothWay(t, f, n + 1, s), r.lot.strength = c > 0 ? c : 1, a.all_strength += r.lot.strength, r.lot.strength_plus = a.all_strength, r.lot.index_of_level = l, r.lot.childs_size = c, l++;
  ...
  placeRelativePosition(t, o, n) {
    ...
    if (this.layoutOptions.from === "left" || this.layoutOptions.from === "right") {
      ...
      f < a && (f = a), f > l && (f = l), o.forEach((h) => {
        ...
        h.fixed !== !0 && (h.lot.level === 0 ? h !== t && (h.lot.y = t.lot.y + f * h.lot.index_of_level) : h.lot.y = t.lot.y + f * (n.max_strength / -2 + h.lot.strengthWithChilds_from + h.lot.strengthWithChilds / 2));
      });
    } else {
      const a = R(this.layoutOptions.min_per_height) || 350, l = R(this.layoutOptions.max_per_height) || 400, r = R(this.layoutOptions.min_per_width) || 250, c = R(this.layoutOptions.max_per_width) || 500;
      let d = Math.round((s.width - 10) / (n.max_strength + 2));
...
const st = (e, i) => {
  at(e);
  let t = null;
  if (e.layoutName === "tree" ? t = new Vt(e, i) : e.layoutName === "center" ? t = new jt(e, i) : e.layoutName === "circle" ? t = new Xt(e, i) : e.layoutName === "force" ? t = new et(e, i) : e.layoutName === "fixed" ? t = new Yt(e, i) : e.layoutName === "smart-tree" && (t = new xe(e, i)), !t)
...
let Le = 0;
const gt = (e) => {
  !e.backgroundImage && (e.backgroundImage = e.backgrounImage), !e.backgroundImageNoRepeat && (e.backgroundImageNoRepeat = e.backgrounImageNoRepeat), typeof e.graphOffset_x == "string" && (e.graphOffset_x = parseInt(e.graphOffset_x)), typeof e.graphOffset_y == "string" && (e.graphOffset_y = parseInt(e.graphOffset_y));
  const i = {
    instanceId: "",
    debug: !0,
    graphLoading: !1,
...
class Pe extends Me {
  ...
  loadLines(t) {
    t.forEach((o) => {
      ...
      const l = `${s.seeks_id}-${a.seeks_id}`, r = `${a.seeks_id}-${s.seeks_id}`, c = ot(o);
      ...
      for (let p = 0; p < d.relations.length; p++)
        if ((d.relations[p].id || c.id) && d.relations[p].id === c.id) {
          _ = !0;
          break;
        }
      _ === !1 && (c.id || (c.id = `${d.seeks_id}-${d.relations.length}`), c.isReverse = f, c.arrow = h, d.relations.push(c)), n && (this.graphData.links.push(d), d.appended = !1);
    });
  }
  ...
  createGraphCanvas(i = "png") {
    return w(this, null, function* () {
      return new Promise((t, o) => {
        ...
        let d = 999999, f = 999999, h = 0, _ = 0;
        const p = 200;
        this.loading("Generating..."), this.graphData.nodes.forEach((x) => {
          x.x < d && (d = x.x), x.x > h && (h = x.x + x.el.offsetWidth), x.y < f && (f = x.y), x.y > _ && (_ = x.y + x.el.offsetHeight);
        });
        const m = d - p, y = f - p;
        ...
        const L = h - d + 200 + p * 2, M = _ - f + 100 + p * 2, z = window.devicePixelRatio;
        this.options.canvasSize.width = L * z, this.options.canvasSize.height = M * z, window.scrollTo(0, 0);
        const E = document.createElement("canvas");
        E.width = L * z, E.height = M * z, E.style.backgroundColor = getComputedStyle(c.parentElement, null).backgroundColor, u("canvas.style.backgroundColor:", E.style.backgroundColor), E.style.width = `${L * z}px`, E.style.height = `${M * z}px`, E.getContext("2d").scale(1, 1);
        const O = {
        ...
        setTimeout(() => {
          this.createImage(c, O, i, "", (x) => {
            this.options.canvasSize.width = s, this.options.canvasSize.height = a, this.options.canvasOffset.x = l, this.options.canvasOffset.y = r, this.options.canvasZoom = n, this.$canvasDom.querySelectorAll(".rel-canvas-slot").forEach((B) => {
              const T = B;
...
class Ee extends Ne {
  ...
  _getJunctionPoint(i, t) {
    return i === X.border ? I.getBorderPoint4MultiLine(...t) : i === X.ltrb ? I.getRectJoinPoint(...t) : i === X.tb ? I.getRectVJoinPoint(...t) : i === X.lr ? I.getRectHJoinPoint(...t) : i === X.left ? I.getRectLeftJoinPoint(...t) : i === X.right ? I.getRectRightJoinPoint(...t) : i === X.top ? I.getRectTopJoinPoint(...t) : i === X.bottom ? I.getRectBottomJoinPoint(...t) : I.getBorderPoint4MultiLine(...t);
  }
  createLinePathByTwoNode(i, t, o, n = 0, s = 1) {
    ...
    let r = i.x, c = i.y, d = t.x, f = t.y;
    const h = { x: 0, y: 0, rotate: 0 };
    if (Number.isNaN(r) || Number.isNaN(c))
      return u("error start node:", i.text, i.x, i.y), h.x = 50, h.y = 50, h.rotate = 0, this.createReturnValue("M 0 0 L 100 100", h);
    if (Number.isNaN(d) || Number.isNaN(f))
      return u("error end point:", t.text, t.x, t.y), h.x = 50, h.y = 50, h.rotate = 0, this.createReturnValue("M 0 0 L 100 100", h);
    ...
    const P = [
      ...
      s,
      n
    ], L = this.options.defaultJunctionPoint || X.border, M = o.fromJunctionPoint || i.junctionPoint || L, z = o.toJunctionPoint || t.junctionPoint || L, E = this._getJunctionPoint(M, P), O = this._getJunctionPoint(z, N);
    if (!E || !O)
      return this.createReturnValue(
        "Unable to calculate the starting point and ending point of the line.",
        h
      );
    const x = E.x, S = E.y, G = O.x, B = O.y;
    if (i === t) {
      const Y = r + _ / 2, C = c + p / 2, W = Math.atan2(S - C, x - Y), D = Math.sin(W + Math.PI) * 5, A = Math.cos(W + Math.PI) * 5, V = { x: x - D, y: S - A }, vt = { x: x + D, y: S + A }, te = x - A * 5, ee = S - D * 5, oe = x - A * 5, ne = S - D * 5, ie = `M${V.x},${V.y} C${te},${ee} ${oe},${ne} ${vt.x},${vt.y}`;
      return h.x = x - A / (A + D) * 20, h.y = S + D / (A + D) * 20, h.rotate = 0, this.createReturnValue(ie, h);
    }
    ...
    if (Number.isNaN(G) || Number.isNaN(B))
      return console.error("error end point:", t), h.x = 50, h.y = 50, h.rotate = 0, this.createReturnValue("M 0 0 L 100 100", h);
    const T = r + _ / 2, j = c + p / 2, b = d + m / 2, k = f + y / 2, tt = this.createLinePathData(
      o,
      h,
      s,
      n,
    ...
    return this.createReturnValue(tt, h);
  }
  createLinePathData(i, t, o, n, s, a, l, r, c, d, f, h, _, p, m, y, P, N) {
    let L = _ - l, M = p - r, z = _ > l ? 1 : -1, E = p > r ? 1 : -1, O = s === "v" ? E : z, x = "";
    if (a === 4) {
      const S = l - c, G = r - d, B = _ - m, T = p - y;
      let j = this.options.defaultPloyLineRadius || 0;
      const b = Math.min(j, Math.abs(L)) * (l < _ ? 1 : -1), k = Math.min(j, Math.abs(M)) * (r < p ? 1 : -1), tt = Math.abs(S) >= f / 2 ? "h" : "v", Y = Math.abs(B) >= P / 2 ? "h" : "v", C = [];
      if (tt === "v") {
        const W = Math.min(50, Math.abs(p - r) / 2), D = G > 0 ? W : -W;
        if (t.x = _, t.y = r + D + (G > 0 ? 20 : -5), C.push(`M ${l} ${r}`), C.push(`v${D - k}`), Y === "v")
          C.push(`c${0},${k},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`h${_ - l - b}`), C.push(`c${b},${0},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${p - r - D - k}`);
        else {
          C.push(`c${0},${k},`), C.push(`${b},${k},`), C.push(`${b},${k}`);
          const A = Math.min(30, Math.abs(_ - l) / 2), V = B > 0 ? -A : A;
          C.push(`h${_ - l + V - b}`), C.push(`c${b},${0},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${p - r - D - k}`), C.push(`h${V - b}`);
        }
      } else {
        const W = Math.min(30, Math.abs(_ - l) / 2), D = S > 0 ? W : -W;
        if (t.x = l + D + (S > 0 ? 20 : -50), t.y = p - 5, C.push(`M ${l} ${r}`), C.push(`h${D - b}`), Y === "v") {
          const A = Math.min(30, Math.abs(p - r) / 2), V = T > 0 ? -A : A;
          C.push(`c${b},${0},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${p - r + V - k}`), C.push(`h${_ - l - D - b}`), C.push(`c${0},${k},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${V - k}`);
        } else
          C.push(`c${b},${0},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${p - r - k * 2}`), C.push(`c${0},${k},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`h${_ - l - D - b}`);
      }
      x = C.join(" ");
    } else if (a === 2 || a === 3 || a === 5 || a === 6 || a === 7) {
      const S = l - c, G = r - d, B = _ - m, T = p - y, j = 1 / (o + 1) * (n + 1);
      let b, k;
      if (a === 2)
        b = s === "v" ? { x: 0, y: O * 30 } : { x: O * 30, y: 0 }, k = s === "v" ? { x: L * j, y: O * -10 } : { x: O * -10, y: M * j };
      else if (a === 3)
        b = s === "v" ? { x: 0, y: M * j } : { x: O * 30, y: 0 }, k = s === "v" ? { x: 0, y: 0 } : { x: O * -10, y: M * j };
      else if (a === 5)
        b = s === "v" ? { x: 0, y: 0 } : { x: 0, y: 0 }, k = s === "v" ? { x: 0, y: M * j } : { x: L * j, y: 0 };
      else if (a === 6) {
        const Y = Math.min(200, Math.max(100, Math.abs(L / 2))), C = Math.min(200, Math.max(100, Math.abs(M / 2))), W = S / (Math.abs(S) + Math.abs(G)) * Y, D = G / (Math.abs(S) + Math.abs(G)) * C;
        b = { x: W, y: D };
        const A = B / (Math.abs(B) + Math.abs(T)) * Y + L, V = T / (Math.abs(B) + Math.abs(T)) * C + M;
        k = { x: A, y: V };
      } else if (a === 7) {
        const W = S / (Math.abs(S) + Math.abs(G)) * 30, D = G / (Math.abs(S) + Math.abs(G)) * 30;
        b = { x: W, y: D };
        const A = B / (Math.abs(B) + Math.abs(T)) * 30 + L, V = T / (Math.abs(B) + Math.abs(T)) * 30 + M;
        k = { x: A, y: V };
      }
      ...
      t.x = tt.x, t.y = tt.y, x = `M${l},${r} c${b.x},${b.y} ${k.x},${k.y} ${L},${M}`;
    } else
  ...
  calcCurveCenter(i, t, o, n, s = 0.5) {
    const a = { x: (1 - s) * i.x + s * t.x, y: (1 - s) * i.y + s * t.y }, l = { x: (1 - s) * t.x + s * o.x, y: (1 - s) * t.y + s * o.y }, r = { x: (1 - s) * o.x + s * n.x, y: (1 - s) * o.y + s * n.y }, c = { x: (1 - s) * a.x + s * l.x, y: (1 - s) * a.y + s * l.y }, d = { x: (1 - s) * l.x + s * r.x, y: (1 - s) * l.y + s * r.y };
    return { x: (1 - s) * c.x + s * d.x, y: (1 - s) * c.y + s * d.y };
  }
...
class Oe extends Ee {
  ...
  showZoomCenter(t, o) {
    ...
    const n = this.options.canvasZoom / 100, s = this.analysisByZoom(n, t), a = (this.options.canvasZoom + o) / 100, l = this.analysisByZoom(a, t), r = a / n, c = 0, d = 0, f = a / n, h = 0, _ = 0;
    this.zoomCenter_of_newSize.x = r * s.NMViewBuff.x + d * s.NMViewBuff.y + h, this.zoomCenter_of_newSize.y = c * s.NMViewBuff.x + f * s.NMViewBuff.y + _;
    ...
    let y = s.NMCanvasStart.x - s.NMCanvasCenter.x, P = s.NMCanvasStart.y - s.NMCanvasCenter.y, N = r * y + d * P + h, L = c * y + f * P + _;
    return l.NMCanvasStart.x = p + s.NMCanvasCenter.x + N, l.NMCanvasStart.y = p + s.NMCanvasCenter.y + L, y = s.NMCanvasEnd.x - s.NMCanvasCenter.x, P = s.NMCanvasEnd.y - s.NMCanvasCenter.y, N = r * y + d * P + h, L = c * y + f * P + _, l.NMCanvasEnd.x = p + s.NMCanvasCenter.x + N, l.NMCanvasEnd.y = p + s.NMCanvasCenter.y + L, s.NMCanvasOffsetBuff.x = p, s.NMCanvasOffsetBuff.y = m, {
      buff_x: p,
      buff_y: m
    };
  }
...
class ze extends Oe {
  ...
  doLayout() {
    return w(this, null, function* () {
      ...
      this.options.showMaskWhenLayouting && this.loading(), yield de(200), this.options.showMaskWhenLayouting && this.clearLoading(), u("node size\uFF1A", this.graphData.nodes.length);
  ...
  animateGoto(i, t, o) {
    return w(this, null, function* () {
      return new Promise((n, s) => {
        u("animateGoto:", i, t);
        const a = i - this.options.canvasOffset.x, l = t - this.options.canvasOffset.y, r = o, c = 5, d = Math.round(a / c), f = Math.round(l / c), h = r / c;
  ...
  animateToZoom(i, t) {
    return w(this, null, function* () {
      return new Promise((o, n) => {
        ...
        u("animateToZoom:", s, r), this.animateStepAction(0, c, l, () => {
...
const bt = (e) => {
  ...
  startDrag(e, i, t, o) {
    ...
    }, ut = t, J = i, q.x = J.x, q.y = J.y, u("[canvas]onDragStart...", U(e), e);
    try {
      const n = bt(e);
      Q.x = n.clientX, Q.y = n.clientY, U(e) ? (document.body.addEventListener("touchmove", H.onNodeMove), document.body.addEventListener("touchend", H.onNodeDragend), e.preventDefault()) : (document.body.addEventListener("mousemove", H.onNodeMove), document.body.addEventListener("mouseup", H.onNodeDragend));
    } catch (n) {
  ...
  onNodeDragend(e) {
    U(e) ? (document.body.removeEventListener("touchmove", H.onNodeMove), document.body.removeEventListener("touchend", H.onNodeDragend)) : (document.body.removeEventListener("mousemove", H.onNodeMove), document.body.removeEventListener("mouseup", H.onNodeDragend)), u("[canvas]onDragend...", U(e), e), ut && ut(
      J.x - q.x,
      J.y - q.y,
      e
    );
...
class Ge extends ze {
  ...
  onNodeClick(t, o) {
    ...
    u("[node]node click", t.text), this.prevClickTime = Date.now(), !o.shiftKey && this.options.disableNodeClickEffect !== !0 && t.disableDefaultClickEffect !== !0 && (this.options.checkedLinkId = "", this.options.checkedLineId = "", this.setCheckedNode(t.id)), this.options.creatingLinePlot && this.onNodeClickWhenCreatingLinePlot(t), this.listeners.onNodeClick && this.listeners.onNodeClick(t, o);
  }
  onLineClick(t, o, n) {
  ...
  startCreatingNodePlot(t, o) {
    ...
    const r = ht(t), c = r.clientX - l.x + 10, d = r.clientY - l.y + 10;
    n || s(c, d);
    const f = (p) => {
      const m = this.$dom.getBoundingClientRect();
      u("[CreatingNodePlot]objectTemplateMove");
      const y = p.clientX - m.x + 10, P = p.clientY - m.y + 10;
      s(y, P);
    }, h = (p) => {
      u("[CreatingNodePlot]user abort!"), this.$dom.removeEventListener("mousemove", f), this.$dom.removeEventListener("click", _), this.$dom.removeEventListener("contextmenu", h), this.options.creatingNodePlot = !1;
    }, _ = (p) => {
      let m = !1;
      if (this.options.creatingNodePlot || (m = !0), this.$dom.removeEventListener("mousemove", f), this.$dom.removeEventListener("click", _), this.$dom.removeEventListener("contextmenu", h), this.options.creatingNodePlot = !1, m) {
        u("[CreatingNodePlot]action be abort!");
        return;
      }
      const y = ht(p), P = y.clientX - l.x + 10, N = y.clientY - l.y + 10;
      if (o && o.disableClickCreate && Math.abs(c - P) < 5 && Math.abs(d - N) < 5) {
        u("[CreatingNodePlot]create node be abort!");
        return;
      }
      const L = this.getCanvasCoordinateByClientCoordinate({ x: y.clientX, y: y.clientY });
      u("[CreatingNodePlot]objectBePlaced:", L), a(L.x, L.y), this._dataUpdated();
    };
    setTimeout(() => {
  ...
  onLineDragStart(t, o) {
    u("onLineDragStart...");
    const n = { x: t.fromNode.x, y: t.fromNode.y }, s = { x: t.toNode.x, y: t.toNode.y }, a = (l, r, c, d, f) => {
    ...
    if (U(t)) {
      ...
      o = (a, l, r, c, d) => {
        ...
        if (f && f.length > 1) {
          ...
          const p = { x: h.clientX, y: h.clientY }, m = { x: _.clientX, y: _.clientY }, y = Math.hypot(n.x - c.x, n.y - c.y), N = Math.hypot(m.x - p.x, m.y - p.y) / y, L = s * N;
          this.setZoom(L), this._dataUpdated();
        } else {
          const _ = h.clientX, p = h.clientY;
          let m = r.x + (_ - c.x), y = r.y + (p - c.y);
          this.options.useHorizontalView ? (m = r.x + (p - c.y), y = r.y - (_ - c.x), this.setCanvasOffset(m, y)) : this.setCanvasOffset(m, y), this._dataUpdated();
        }
      };
    } else
      o = (n, s, a, l, r) => {
        const c = a.x + n, d = a.y + s;
        this.setCanvasOffset(c, d), this._dataUpdated();
      };
    H.startDrag(t, this.options.canvasOffset, (...n) => {
  ...
  onNodeDragStart(t, o) {
    ...
    H.startDrag(o, t, n, (s, a, l) => {
      let r = s / (this.options.canvasZoom / 100) + l.x, c = a / (this.options.canvasZoom / 100) + l.y;
      if (this.listeners.onNodeDragging) {
        const d = this.listeners.onNodeDragging(t, r, c, o);
        d && (typeof d.x == "number" && (r = d.x), typeof d.y == "number" && (c = d.y));
      }
      this.setNodePosition(t, r, c), this.options.useHorizontalView ? (r = a / (this.options.canvasZoom / 100) + l.x, c = -s / (this.options.canvasZoom / 100) + l.y, this.setNodePosition(t, r, c)) : this.setNodePosition(t, r, c), this.updateElementLines(), this._dataUpdated();
    });
  }
...
class Be extends Ge {
  ...
  appendJsonData(i, t = !0) {
    return w(this, null, function* () {
      u("appendData:", i), this.loadGraphJsonData(i), t && (this.disableNextLayoutAnimation = !0, yield this.doLayout());
    });
  }
}
class Ut extends Be {
...
var Ue = function() {
  var e = this, i = e.$createElement, t = e._self._c || i;
  return t("g", [t("path", { staticClass: "c-rg-line", class: [
  ...
  }, attrs: { d: e.pathData.path, stroke: e.relation.color ? e.relation.color : e.options.defaultLineColor, "marker-start": e.showStartArrow, "marker-end": e.showEndArrow, fill: "none" }, on: { touchstart: function(o) {
    return e.onClick(e.relation, o);
  }, click: function(o) {
    return e.onClick(e.relation, o);
  } } }), e.options.defaultShowLineLabel && e.options.canvasZoom > 40 ? t("g", { attrs: { transform: e.pathData.textTransform } }, [t("text", { key: "t-" + e.relation.seeks_id, staticClass: "c-rg-line-text", class: { "c-rg-line-text-checked": e.checked }, style: {
    opacity: e.relation.opacity,
  ...
  }, attrs: { x: e.relation.textOffset_x || e.options.defaultLineTextOffset_x || 0, y: e.relation.textOffset_y || e.options.defaultLineTextOffset_y || 10, "text-anchor": "middle" }, on: { touchstart: function(o) {
    return e.onClick(e.relation, o);
  }, click: function(o) {
    return e.onClick(e.relation, o);
...
const Zt = /* @__PURE__ */ function() {
  ...
  computed: {
    textOffset() {
      const e = this.relation.textOffset_x || this.options.defaultLineTextOffset_x || 0, i = this.relation.textOffset_y || this.options.defaultLineTextOffset_y || -8;
    ...
    textHPosition() {
      if (this.relation.lineShape === 4 || this.options.defaultLineShape === 4)
        if (this.options.layoutDirection === "v") {
          const e = this.link.fromNode.x, i = this.link.toNode.x;
          return Math.abs(i - e) + 43;
        } else {
          const e = this.link.fromNode.y, i = this.link.toNode.y;
          return Math.abs(i - e) + 43;
        }
      return "50%";
    },
...
var Ke = function() {
  var e = this, i = e.$createElement, t = e._self._c || i;
  return t("g", [t("use", { staticClass: "c-rg-line", class: [
    e.relation.styleClass,
    e.relation.dashType ? "rg-line-dashtype-" + e.relation.dashType : void 0,
    e.relation.animation ? "rg-line-anm-" + e.relation.animation : void 0,
    e.checked ? "c-rg-line-checked" : void 0
    ...
    return e.onClick(e.relation, o);
  } } }), t("g", { attrs: { transform: e.textOffset } }, [t("text", { staticClass: "c-rg-line-text", class: { "c-rg-line-text-checked": e.checked }, style: {
    opacity: e.relation.opacity,
    fill: e.relation.fontColor ? e.relation.fontColor : e.options.defaultLineFontColor ? e.options.defaultLineFontColor : e.relation.color ? e.relation.color : e.options.defaultLineColor
  }, attrs: { rotate: e.textRotate }, on: { touchstart: function(o) {
    return e.onClick(e.relation, o);
  }, click: function(o) {
    return e.onClick(e.relation, o);
  } } }, [t("textPath", { attrs: { "xlink:href": e.pathRef, startOffset: e.textHPosition, "text-anchor": e.textAnchor, method: "align", spacing: "auto" } }, [e._v(" " + e._s(e.relation.text) + " ")])])])]);
}, to = [];
const Pt = {};
var eo = /* @__PURE__ */ F(
  Qe,
...
var so = function() {
  var e = this, i = e.$createElement, t = e._self._c || i;
  return e.linkProps.isHide !== !0 && e.isAllowShowNode(e.linkProps.fromNode) && e.isAllowShowNode(e.linkProps.toNode) ? t("g", { ref: "seeksRGLink", staticClass: "rel-link-peel", class: [e.options.checkedLinkId == e.linkProps.seeks_id ? "c-rg-link-checked" : ""], attrs: { transform: "translate(0,0)", "data-id": e.linkProps.seeks_id } }, [e._l(e.linkProps.relations, function(o, n) {
    return [e.options.oldVueVersion && !e.options.ovUseLineSlot ? [(e.options.lineUseTextPath || o.useTextPath) && o.isHide === !1 ? t("RGLineTextByPath", { key: "l-" + o.id, attrs: { link: e.linkProps, relation: o, "relation-index": n } }) : o.isHide === !1 ? t("RGLineSmart", { key: "l-" + o.id, attrs: { link: e.linkProps, relation: o, "relation-index": n } }) : e._e()] : e._t("line", function() {
      return [(e.options.lineUseTextPath || o.useTextPath) && o.isHide === !1 ? t("RGLineTextByPath", { key: "l-" + o.id, attrs: { link: e.linkProps, relation: o, "relation-index": n } }) : o.isHide === !1 ? t("RGLineSmart", { key: "l-" + o.id, attrs: { link: e.linkProps, relation: o, "relation-index": n } }) : e._e()];
...
var Co = function() {
  ...
  return t("div", { staticClass: "rel-single-graph", staticStyle: { overflow: "visible" } }, [t("div", { ref: "rgCanvas", staticClass: "rel-linediv" }, [t("svg", { staticClass: "rel-lines-svg", style: { width: e.options.canvasSize.width + "px", height: e.options.canvasSize.height + "px" }, attrs: { xmlns: "http://www.w3.org/2000/svg" } }, [t("defs", [t("linearGradient", { attrs: { id: "my-lineStyle", gradientUnits: "objectBoundingBox", cx: "0.5", cy: "0.5" } }, [t("stop", { attrs: { offset: "0%", "stop-color": "#e52c5c", "stop-opacity": "1" } }), t("stop", { attrs: { offset: "100%", "stop-color": "#FD8B37", "stop-opacity": "0" } })], 1), t("marker", { attrs: { id: e.options.instanceId + "-arrow-default", markerWidth: e.options.defaultLineMarker.markerWidth, markerHeight: e.options.defaultLineMarker.markerHeight, refX: e.options.defaultLineMarker.refX, refY: e.options.defaultLineMarker.refY, "marker-units": "strokeWidth", orient: "auto", viewBox: "0 0 12 12" } }, [t("path", { style: { fill: e.options.defaultLineColor }, attrs: { d: e.options.defaultLineMarker.data } })]), t("marker", { attrs: { id: e.options.instanceId + "-start-arrow-default", markerWidth: e.options.defaultLineMarker.markerWidth, markerHeight: e.options.defaultLineMarker.markerHeight, refX: e.options.defaultLineMarker.refX, refY: e.options.defaultLineMarker.refY, "marker-units": "strokeWidth", orient: "auto", viewBox: "0 0 12 12" } }, [t("path", { style: { fill: e.options.defaultLineColor }, attrs: { d: e.options.defaultLineMarker.data, transform: "translate(12,12)rotate(180)" } })]), t("marker", { attrs: { id: e.options.instanceId + "-arrow-checked", markerWidth: e.options.defaultLineMarker.markerWidth, markerHeight: e.options.defaultLineMarker.markerHeight, refX: e.options.defaultLineMarker.refX, refY: e.options.defaultLineMarker.refY, "marker-units": "strokeWidth", orient: "auto", viewBox: "0 0 12 12" } }, [t("path", { style: { fill: e.options.checkedLineColor }, attrs: { d: e.options.defaultLineMarker.data } })]), t("marker", { attrs: { id: e.options.instanceId + "-start-arrow-checked", markerWidth: e.options.defaultLineMarker.markerWidth, markerHeight: e.options.defaultLineMarker.markerHeight, refX: e.options.defaultLineMarker.refX, refY: e.options.defaultLineMarker.refY, "marker-units": "strokeWidth", orient: "auto", viewBox: "0 0 12 12" } }, [t("path", { style: { fill: e.options.checkedLineColor }, attrs: { d: e.options.defaultLineMarker.data, transform: "translate(12,12)rotate(180)" } })]), e._l(e.allLineColors, function(o) {
    ...
    return t("SeeksRGLink", { key: o.seeks_id, attrs: { "link-props": o }, scopedSlots: e._u([{ key: "line", fn: function(n) {
      var s = n.line;
      return [e._t("line", null, { line: s, link: o })];
    } }], null, !0) });
  }), e.options.creatingLinePlot && e.options.newLinkTemplate.fromNode ? t("RGLineSmart", { key: "s-line-template", attrs: { link: e.options.newLinkTemplate, relation: e.options.newLineTemplate, "relation-index": 0 } }) : e._e()], 2)]), t("div", { staticClass: "rel-nodediv rel-nodediv-for-webkit" }, e._l(e.graphData.nodes, function(o) {
...
var Po = function() {
  var e = this, i = e.$createElement, t = e._self._c || i;
  return t("div", { staticClass: "rel-map", class: [e.options.layoutClassName, e.options.backgroundImageNoRepeat ? "rel-map-background-norepeat" : void 0], style: { width: "100%", height: "100%", "background-color": e.options.backgroundColor || void 0, "background-image": e.options.backgroundImage ? "url(" + e.options.backgroundImage + ")" : void 0 }, on: { contextmenu: function(o) {
...
const _n = /* @__PURE__ */ function() {
  ...
  created() {
    ...
    Z.version.slice(0, 4) === "2.5." && (e = !0), Z.version.slice(0, 4) === "2.6." && Number.parseInt(Z.version.split(".")[2]) <= 12 && (e = !0), e && (this.oldVueVersion = !0, console.error(`\u60A8\u7684Vue\u7248\u672C\uFF1A${Z.version}\u6CE8\u610F\uFF1A\u5F53\u4F60\u4F7F\u7528\u7684vue\u7248\u672C\u7B49\u4E8E\u4F4E\u4E8E2.6.12\u65F6\uFF0C\u56FE\u8C31\u4F1A\u663E\u793A\u4E0D\u6B63\u5E38\uFF0C\u53C2\u8003\u8FD9\u4E2A\u8FDE\u63A5\u89E3\u51B3\u8FD9\u4E2A\u95EE\u9898\uFF1Ahttps://github.com/seeksdream/relation-graph/issues/113`)), K || console.error("[relation-graph]Please introduce component screenfull, for example:https://cdnjs.cloudflare.com/ajax/libs/screenfull.js/5.1.0/screenfull.min.js"), Ht || console.error("[relation-graph]Please introduce component html2canvas, for example:https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js");
  },
  mounted() {
    u("---------------------------graph mounted---------------------------");
    const e = this.relationGraphCore ? new this.relationGraphCore(this.options, It(this)) : new Ut(this.options, It(this));
...
const Qt = /* @__PURE__ */ function() {
  ...
  FixedLayouter: Yt,
  ForceLayouter: et
}, Pn = Ce, Nn = ke, En = ge, On = pe, zn = I, $n = ye, Gn = $e, Kt = (e, i) => {
  e.component("RelationGraph", Qt);
};
typeof window != "undefined" && window.Vue && Kt(window.Vue);
const wn = Qt, Bn = Ct(wt({}, wn), {
  version: Wt,
  install: Kt
});
export {
