...
(function(e) {
  ...
  i = function() {
    var d, f, c, m;
    (m = document.createElement("div")).innerHTML = l, l = null, (c = m.getElementsByTagName("svg")[0]) && (c.setAttribute("aria-hidden", "true"), c.style.position = "absolute", c.style.width = 0, c.style.height = 0, c.style.overflow = "hidden", d = c, (f = document.body).firstChild ? (m = d, (c = f.firstChild).parentNode.insertBefore(m, c)) : f.appendChild(d));
...
const de = (e) => (e = e.replace("#", ""), e = e.replace("(", ""), e = e.replace(")", ""), e = e.replace(/,/, "-"), e), Y = (e) => {
...
}, me = ["arrow", "id", "reverseText", "isReverse"], mt = (e, i) => {
  !e || e.relations.forEach((t) => {
    const o = {};
    Object.keys(t).forEach((n) => {
      me.includes(n) || t[n] !== void 0 && (o[n] = t[n]);
    }), i.push(o);
  });
}, U = {
  border: "border",
  ...
  isRectangleOverlap(e, i) {
    const t = e.x + D, o = i.x + D, n = e.el.offsetWidth - D * 2, s = i.el.offsetWidth - D * 2, a = e.y + D, l = i.y + D, r = e.el.offsetHeight - D * 2, h = i.el.offsetHeight - D * 2;
  ...
  getNoOverlapLimitedPosition(e, i, t, o) {
    const n = e.x + D, s = e.y + D, a = i + D, l = o.x + D, r = e.el.offsetWidth - D * 2, h = o.el.offsetWidth - D * 2, d = t + D, f = o.y + D, c = e.el.offsetHeight - D * 2, m = o.el.offsetHeight - D * 2;
    let p = e.x, _ = e.y;
    const y = this.isXOverlap(n, l, r, h), M = this.isYOverlap(s, f, c, m);
    return y ? d < f ? (p = i, _ = f - c - D) : d > f && (p = i, _ = f + m - D) : M && (a < l ? (p = l - r - D, _ = t) : a > l && (p = l + h - D, _ = t)), { x: p, y: _ };
  },
...
}, D = 8, ye = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  RGNodesAnalytic: g,
  default: g
...
class jt extends it {
  ...
  analysisNodes4Didirectional(t, o, n, s, a) {
    ...
    o.forEach((d) => {
      ...
      let m = 0;
      c.forEach((p) => {
        p.lot || (p.lot = { eached: !1, childs: [] }), p.lot.eached || (p.lot.parent = d, g.isAllowShowNode(p) ? (p.lot.eached = !0, p.lot.index_of_parent = m++, d.lot.childs.push(p), r.push(p), f++) : d.lot.childs.push(p));
...
function ve(e, i, t, o, n, s, a, l, r) {
  const h = n - e, d = s - i, f = Math.sqrt(h * h + d * d), c = r * d / f, m = -(r * h) / f, p = e + c, _ = i + m, y = e - t / 2, M = e + t / 2, N = i - o / 2, L = i + o / 2;
  let P = [];
  if (h !== 0) {
    let E = (y - p) / h, x = _ + E * d;
    x >= N && x <= L && P.push({ x: y, y: x });
    let S = (M - p) / h, z = _ + S * d;
    z >= N && z <= L && P.push({ x: M, y: z });
  }
  if (d !== 0) {
    let E = (N - _) / d, x = p + E * h;
    x >= y && x <= M && P.push({ x, y: N });
    let S = (L - _) / d, z = p + S * h;
    z >= y && z <= M && P.push({ x: z, y: L });
  }
  let $ = null, O = 1 / 0;
  return P.forEach((E) => {
    const x = Math.sqrt((E.x - n) ** 2 + (E.y - s) ** 2);
    x < O && (O = x, $ = E);
  }), $;
}
const I = {
  getRectPoint(e, i, t, o, n, s, a, l, r = !1, h = 1, d = 1) {
    r && (d = h - d - 1);
    const f = d - (h - 1) / 2, c = e + n / 2, m = i + s / 2, p = t + a / 2, _ = o + l / 2;
    if (h > 1) {
      const x = 14 * f, S = ve(
        c,
        m,
    ...
    const y = c < p ? 1 : -1, M = m < _ ? 1 : -1;
    if (_ === m)
      return { x: c + y * n / 2, y: m };
    const N = Math.abs((p - c) / (_ - m)), L = n / s;
    let P = 0, $ = 0;
    N < L ? (P = y * s / 2 * N + M * L, $ = M * s / 2) : (P = y * n / 2, $ = M * n / 2 / N + y * L);
    const O = c + P, E = m + $;
    return { x: O, y: E };
  },
  getRectPointBasic(e, i, t, o, n, s, a, l) {
    ...
    const m = n / l;
    let p = 0, _ = 0, y = "1";
    return -1 * m < c && c < m ? (y = "2", h < f ? (p = s / 2 * c, _ = s / 2) : (p = -1 * s / 2 * c, _ = -1 * s / 2)) : (r < d ? (p = 1 * n / 2, _ = 1 * n / 2 / c) : (p = -1 * n / 2, _ = -1 * n / 2 / c), y = "3"), { x: r + p, y: h + _, _case: y };
  },
  ...
  getBorderPoint4MultiLine(e, i, t, o, n, s, a, l, r, h, d, f) {
    return r === 0 ? this.getCirclePoint4MultiLine(e, i, t, o, n, s, a, l, h, d, f) : this.getRectPoint(e, i, t, o, n, s, a, l, h, d, f);
  },
  getCirclePoint(e, i, t, o, n, s, a, l) {
    const r = t + a / 2, h = o + l / 2, d = e + n / 2, f = i + s / 2, c = r - d;
    if (c === 0)
      return { x: d, y: f - s / 2 * (h < f ? 1 : -1) };
    const p = (h - f) / c, _ = Math.sqrt(1 / (1 / (n / 2) ** 2 + p ** 2 / (s / 2) ** 2)) * (r < d ? 1 : -1), y = p * _;
    return { x: d - _, y: f - y };
  },
  getCirclePoint4MultiLine(e, i, t, o, n, s, a, l, r, h, d, f = 0) {
    r && (d = h - d - 1);
    const c = t + a / 2, m = o + l / 2, p = e + n / 2, _ = i + s / 2, y = c - p;
    if (y === 0)
      return { x: p, y: _ - s / 2 * (m < _ ? 1 : -1) };
    const N = (d - (h - 1) / 2) * 14, L = m - _, P = Math.sqrt(y ** 2 + L ** 2) * N / y, $ = L / y, O = n / 2, E = s / 2, x = c < p ? 1 : -1, S = (-1 * O ** 2 * $ * P + O * E * Math.sqrt(E ** 2 + $ ** 2 * O ** 2 - P ** 2) / x) / (E ** 2 + O ** 2 * $ ** 2), z = $ * S + P;
    return f === 0 && (Number.isNaN(S) || Number.isNaN(z)) ? this.getCirclePoint4MultiLine(e, i, t, o, n, s, a, l, r, 1, 0, f + 1) : { x: p - S, y: _ - z };
  },
  getCirclePointBasic(e, i, t, o, n, s, a, l, r) {
    const h = e + n / 2, d = i + s / 2, f = t + a / 2, c = o + l / 2, m = f - h, p = c - d, _ = Math.sqrt(m * m + p * p), y = _ - r, M = y * m / _ * -1, N = y * p / _ * -1;
    return { x: f + M, y: c + N };
  },
  getCirclePointPlus(e, i, t, o, n, s, a, l) {
    const r = e + n / 2, h = i + s / 2, d = t + a / 2, f = o + l / 2, c = d - r, m = f - h, p = Math.sqrt(c * c + m * m), _ = (p - n / 2) * c / p * -1, y = (p - s / 2) * m / p * -1;
    return { x: d + _, y: f + y };
  },
  getOvalPoint(e, i, t, o, n) {
    return {
      x: e + t * Math.sin((bt + o * (360 / n) + 0) * Math.PI / 180),
      y: i + t * Math.cos((bt + o * (360 / n) + 0) * Math.PI / 180) * -1
    };
  },
  getAngleType(e, i) {
    if (e >= 0 && i >= 0)
...
class et extends it {
  ...
  easyPlaceRelativePosition(t, o) {
    o.forEach((s) => {
      ...
      const a = s.lot.subling;
      if (a && a.level > 1) {
        const l = s.lot.parent.lot.strength_plus - s.lot.parent.lot.strength, h = (s.lot.parent.lot.strength_plus - l) / (s.lot.parent.lot.childs.length + 1) * (s.lot.index_of_parent + 1), d = I.getOvalPoint(t.lot.x, t.lot.y, (a.level - 1) * 40 + 50, l + h, s.lot.parent.lot.subling.all_strength);
      ...
      for (const n in this.visibleNodes) {
        ...
        if (!(this.justLayoutSingleNode && !s.singleNode) && !s.fixed && s.lot && s.lot.placed === !0)
          for (const a in this.visibleNodes) {
            ...
            l.lot && l.lot.placed === !0 && n !== a && this.addGravityByNode(s, l);
          }
      }
    if (this.byLine)
      if (this.allLinks && this.allLinks.length > 0) {
        ...
        for (const n of this.visibleNodes)
          n.lot && n.lot.parent && this.addElasticByLine(
            n.lot.parent,
            n
          );
    for (const n in this.visibleNodes)
  ...
  placeRelativePosition(t, o, n) {
    ...
    o.forEach((f) => {
      ...
      if (c && c.level === 1) {
        ...
        const m = this.getLevelR(a, c.level) || c.level * h, p = I.getOvalPoint(t.lot.x, t.lot.y, m, f.lot.strength_plus - f.lot.strength / 2, c.all_strength);
    ...
    o.forEach((f) => {
      const c = f.lot.subling;
      if (c && c.level > 1) {
        const m = f.lot.parent.lot.strength_plus - f.lot.parent.lot.strength, _ = (f.lot.parent.lot.strength_plus - m) / (f.lot.parent.lot.childs_size + 1) * (f.lot.index_of_parent + 1), y = this.getLevelR(a, c.level) || (c.level - 1) * d + h, M = I.getOvalPoint(t.lot.x, t.lot.y, y, m + _, f.lot.parent.lot.subling.all_strength);
...
class xe extends it {
  ...
  analysisNodes4Didirectional(t, o, n, s) {
    ...
    const l = Object.keys(a).map((h) => Number.parseInt(h)).sort(), r = (h) => {
      ...
      let c = 0;
      f.forEach((m) => {
        m.lot.parent && c < m.lot.parent.lot.strengthWithChilds_from && (c = m.lot.parent.lot.strengthWithChilds_from), h === 0 ? c += 1 : m.lot.strengthWithChilds_from = c, u("levels:", h, c, m.text, m.lot.strengthWithChilds), c += m.lot.strengthWithChilds;
      });
    };
...
const st = (e, i) => {
  at(e);
  let t = null;
  if (e.layoutName === "tree" ? t = new jt(e, i) : e.layoutName === "center" ? t = new Ut(e, i) : e.layoutName === "circle" ? t = new Xt(e, i) : e.layoutName === "force" ? t = new et(e, i) : e.layoutName === "fixed" ? t = new Yt(e, i) : e.layoutName === "smart-tree" && (t = new xe(e, i)), !t)
...
const gt = (e) => {
  ...
  const i = {
    ...
    creatingNodePlot: !1,
    showTemplateNode: !0,
    newNodeTemplate: nt(JSON.parse(JSON.stringify(_t))),
    creatingLinePlot: !1,
    newLineTemplate: {
...
class Pe extends Me {
  ...
  loadLines(t) {
    t.forEach((o) => {
      ...
      m === !1 && (h.id || (h.id = `${d.seeks_id}-${d.relations.length}`), h.isReverse = f, h.arrow = c, d.relations.push(h)), n && (this.graphData.links.push(d), d.appended = !1);
    });
  }
  ...
  createGraphCanvas(i = "png") {
    return w(this, null, function* () {
      return new Promise((t, o) => {
        ...
        let d = 999999, f = 999999, c = 0, m = 0;
        const p = 200;
        this.loading("Generating..."), this.graphData.nodes.forEach((x) => {
          x.x < d && (d = x.x), x.x > c && (c = x.x + x.el.offsetWidth), x.y < f && (f = x.y), x.y > m && (m = x.y + x.el.offsetHeight);
        });
        const _ = d - p, y = f - p;
        ...
        const L = c - d + 200 + p * 2, P = m - f + 100 + p * 2, $ = window.devicePixelRatio;
        this.options.canvasSize.width = L * $, this.options.canvasSize.height = P * $, window.scrollTo(0, 0);
        const O = document.createElement("canvas");
        O.width = L * $, O.height = P * $, O.style.backgroundColor = getComputedStyle(h.parentElement, null).backgroundColor, u("canvas.style.backgroundColor:", O.style.backgroundColor), O.style.width = `${L * $}px`, O.style.height = `${P * $}px`, O.getContext("2d").scale(1, 1);
        const E = {
        ...
        setTimeout(() => {
          this.createImage(h, E, i, "", (x) => {
            this.options.canvasSize.width = s, this.options.canvasSize.height = a, this.options.canvasOffset.x = l, this.options.canvasOffset.y = r, this.options.canvasZoom = n, this.$canvasDom.querySelectorAll(".rel-canvas-slot").forEach((G) => {
              const T = G;
              T.style.left = "0px", T.style.top = "0px";
            }), this.graphData.nodes.forEach((G) => {
              G.x = G.x + _, G.y = G.y + y;
            }), this.$canvasDom.querySelectorAll(".rel-lines-svg-el-lines").forEach((G) => {
              const T = G;
...
class Oe extends Ee {
  ...
  createLinePathByTwoNode(i, t, o, n = 0, s = 1) {
    ...
    let m = i.el.offsetWidth || i.width || 60, p = i.el.offsetHeight || i.height || 60;
    if (Number.isNaN(m) || Number.isNaN(p))
      return c.x = 50, c.y = 50, c.rotate = 0, this.createReturnValue("M 0 0 L -10 10", c);
    let _ = t.el.offsetWidth || t.width || 60, y = t.el.offsetHeight || t.height || 60;
    if (Number.isNaN(_) || Number.isNaN(y))
      return c.x = 50, c.y = 50, c.rotate = 0, this.createReturnValue("M 0 0 L 10 10", c);
    o.isReverse && ([r, h, d, f, m, p, _, y] = [
      d,
      f,
      r,
      h,
    ...
    const M = [
      ...
      s,
      n
    ], L = this.options.defaultJunctionPoint || U.border, P = o.fromJunctionPoint || i.junctionPoint || L, $ = o.toJunctionPoint || t.junctionPoint || L, O = this._getJunctionPoint(P, M), E = this._getJunctionPoint($, N);
    if (!O || !E)
      return this.createReturnValue(
        "Unable to calculate the starting point and ending point of the line.",
        c
      );
    const x = O.x, S = O.y, z = E.x, G = E.y;
    if (i === t) {
      const X = r + m / 2, C = h + p / 2, W = Math.atan2(S - C, x - X), B = Math.sin(W + Math.PI) * 5, A = Math.cos(W + Math.PI) * 5, V = { x: x - B, y: S - A }, vt = { x: x + B, y: S + A }, ee = x - A * 5, oe = S - B * 5, ne = x - A * 5, ie = S - B * 5, se = `M${V.x},${V.y} C${ee},${oe} ${ne},${ie} ${vt.x},${vt.y}`;
      return c.x = x - A / (A + B) * 20, c.y = S + B / (A + B) * 20, c.rotate = 0, this.createReturnValue(se, c);
    }
    ...
    if (Number.isNaN(z) || Number.isNaN(G))
      return u("error end point:", t.text), c.x = 50, c.y = 50, c.rotate = 0, this.createReturnValue("M 0 0 L 10 0", c);
    const T = r + m / 2, j = h + p / 2, b = d + _ / 2, k = f + y / 2, tt = this.createLinePathData(
      o,
      c,
      s,
      n,
  ...
  createLinePathData(i, t, o, n, s, a, l, r, h, d, f, c, m, p, _, y, M, N) {
    let L = m - l, P = p - r, $ = m > l ? 1 : -1, O = p > r ? 1 : -1, E = s === "v" ? O : $, x = "";
    if (a === 4) {
      const S = l - h, z = r - d, G = m - _, T = p - y;
      let j = this.options.defaultPloyLineRadius || 0;
      const b = Math.min(j, Math.abs(L)) * (l < m ? 1 : -1), k = Math.min(j, Math.abs(P)) * (r < p ? 1 : -1), tt = Math.abs(S) >= f / 2 ? "h" : "v", X = Math.abs(G) >= M / 2 ? "h" : "v", C = [];
      if (tt === "v") {
        const W = i.polyLineStartDistance || Math.max(Math.min(30, Math.abs(p - r) / 2), 15), B = z > 0 ? W : -W;
        if (t.x = m, t.y = r + B + (z > 0 ? 20 : -5), C.push(`M ${l} ${r}`), C.push(`v${B - k}`), X === "v")
          C.push(`c${0},${k},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`h${m - l - b * 2}`), C.push(`c${b},${0},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${p - r - B - k}`);
        else {
          C.push(`c${0},${k},`), C.push(`${b},${k},`), C.push(`${b},${k}`);
          const A = Math.min(30, Math.abs(m - l) / 2), V = G > 0 ? -A : A;
          C.push(`h${m - l + V - b}`), C.push(`c${b},${0},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${p - r - B - k}`), C.push(`h${V - b}`);
        }
      } else {
        const W = i.polyLineStartDistance || Math.max(Math.min(30, Math.abs(m - l) / 2), 15), B = S > 0 ? W : -W;
        if (t.x = l + B + (S > 0 ? 20 : -50), t.y = p - 5, C.push(`M ${l} ${r}`), C.push(`h${B - b}`), X === "v") {
          const A = Math.min(30, Math.abs(p - r) / 2), V = T > 0 ? -A : A;
          C.push(`c${b},${0},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${p - r + V - k}`), C.push(`h${m - l - B - b}`), C.push(`c${0},${k},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${V - k}`);
        } else
          C.push(`c${b},${0},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`v${p - r - k * 2}`), C.push(`c${0},${k},`), C.push(`${b},${k},`), C.push(`${b},${k}`), C.push(`h${m - l - B - b}`);
      }
      x = C.join(" ");
    } else if (a === 2 || a === 3 || a === 5 || a === 6 || a === 7) {
      const S = l - h, z = r - d, G = m - _, T = p - y, j = 1 / (o + 1) * (n + 1);
      let b, k;
      if (a === 2)
        b = s === "v" ? { x: 0, y: E * 30 } : { x: E * 30, y: 0 }, k = s === "v" ? { x: L * j, y: E * -10 } : { x: E * -10, y: P * j };
      else if (a === 3)
        b = s === "v" ? { x: 0, y: P * j } : { x: E * 30, y: 0 }, k = s === "v" ? { x: 0, y: 0 } : { x: E * -10, y: P * j };
      else if (a === 5)
        b = s === "v" ? { x: 0, y: 0 } : { x: 0, y: 0 }, k = s === "v" ? { x: 0, y: P * j } : { x: L * j, y: 0 };
      else if (a === 6) {
        const X = Math.min(200, Math.max(100, Math.abs(L / 2))), C = Math.min(200, Math.max(100, Math.abs(P / 2))), W = S / (Math.abs(S) + Math.abs(z)) * X, B = z / (Math.abs(S) + Math.abs(z)) * C;
        b = { x: W, y: B };
        const A = G / (Math.abs(G) + Math.abs(T)) * X + L, V = T / (Math.abs(G) + Math.abs(T)) * C + P;
        k = { x: A, y: V };
      } else if (a === 7) {
        const W = S / (Math.abs(S) + Math.abs(z)) * 30, B = z / (Math.abs(S) + Math.abs(z)) * 30;
        b = { x: W, y: B };
        const A = G / (Math.abs(G) + Math.abs(T)) * 30 + L, V = T / (Math.abs(G) + Math.abs(T)) * 30 + P;
        k = { x: A, y: V };
      }
      ...
      t.x = tt.x, t.y = tt.y, x = `M${l},${r} c${b.x},${b.y} ${k.x},${k.y} ${L},${P}`;
    } else
      t.rotate = I.getTextAngle(l, r, m, p), t.x = Math.round(l + (m - l) / 2), t.y = Math.round(r + (p - r) / 2), Number.isNaN(t.rotate) && (t.rotate = 0), x = `M ${l} ${r} L ${m} ${p}`;
    return x;
  }
...
class Ne extends Oe {
  ...
  showZoomCenter(t, o) {
    ...
    const n = this.options.canvasZoom / 100, s = this.analysisByZoom(n, t), a = (this.options.canvasZoom + o) / 100, l = this.analysisByZoom(a, t), r = a / n, h = 0, d = 0, f = a / n, c = 0, m = 0;
    this.zoomCenter_of_newSize.x = r * s.NMViewBuff.x + d * s.NMViewBuff.y + c, this.zoomCenter_of_newSize.y = h * s.NMViewBuff.x + f * s.NMViewBuff.y + m;
    const p = s.NMViewBuff.x - this.zoomCenter_of_newSize.x, _ = s.NMViewBuff.y - this.zoomCenter_of_newSize.y;
    this.zoomCenter_of_newSize.x += s.NMCanvasCenter.x, this.zoomCenter_of_newSize.y += s.NMCanvasCenter.y;
    let y = s.NMCanvasStart.x - s.NMCanvasCenter.x, M = s.NMCanvasStart.y - s.NMCanvasCenter.y, N = r * y + d * M + c, L = h * y + f * M + m;
    return l.NMCanvasStart.x = p + s.NMCanvasCenter.x + N, l.NMCanvasStart.y = p + s.NMCanvasCenter.y + L, y = s.NMCanvasEnd.x - s.NMCanvasCenter.x, M = s.NMCanvasEnd.y - s.NMCanvasCenter.y, N = r * y + d * M + c, L = h * y + f * M + m, l.NMCanvasEnd.x = p + s.NMCanvasCenter.x + N, l.NMCanvasEnd.y = p + s.NMCanvasCenter.y + L, s.NMCanvasOffsetBuff.x = p, s.NMCanvasOffsetBuff.y = _, {
      buff_x: p,
      buff_y: _
    };
  }
...
class ze extends Ne {
  ...
  doLayout() {
    return w(this, null, function* () {
      ...
      this.options.showMaskWhenLayouting && this.loading(), yield Lt(200), this.options.showMaskWhenLayouting && this.clearLoading(), u("node size\uFF1A", this.graphData.nodes.length);
...
const kt = (e) => {
  ...
  startDrag(e, i, t, o) {
    ...
    try {
      const n = kt(e);
      Q.x = n.clientX, Q.y = n.clientY, Y(e) ? (document.body.addEventListener("touchmove", H.onNodeMove), document.body.addEventListener("touchend", H.onNodeDragend), e.preventDefault()) : (document.body.addEventListener("mousemove", H.onNodeMove), document.body.addEventListener("mouseup", H.onNodeDragend));
    } catch (n) {
  ...
  onNodeDragend(e) {
    Y(e) ? (document.body.removeEventListener("touchmove", H.onNodeMove), document.body.removeEventListener("touchend", H.onNodeDragend)) : (document.body.removeEventListener("mousemove", H.onNodeMove), document.body.removeEventListener("mouseup", H.onNodeDragend)), u("[canvas]onDragend...", Y(e), e), ut && ut(
      J.x - q.x,
      J.y - q.y,
      e
    );
...
class De extends ze {
  ...
  startCreatingNodePlot(t, o) {
    this.options.newNodeTemplate = nt(JSON.parse(JSON.stringify(_t))), this.options.creatingNodePlot = !0;
    const n = Y(t);
    this.options.showTemplateNode = !n;
    let s = (p, _) => {
      this.options.newNodeTemplate.x = p, this.options.newNodeTemplate.y = _, this._dataUpdated();
    }, a = (p, _) => {
      const y = "s-" + Date.now();
      this.addNodes([{
        id: y,
    ...
    const f = (p) => {
      ...
      const y = p.clientX - _.x + 10, M = p.clientY - _.y + 10;
      s(y, M);
    }, c = (p) => {
      u("[CreatingNodePlot]user abort!"), this.$dom.removeEventListener("mousemove", f), this.$dom.removeEventListener("click", m), this.$dom.removeEventListener("contextmenu", c), this.options.creatingNodePlot = !1;
    }, m = (p) => {
      let _ = !1;
      if (this.options.creatingNodePlot || (_ = !0), this.$dom.removeEventListener("mousemove", f), this.$dom.removeEventListener("click", m), this.$dom.removeEventListener("contextmenu", c), this.options.creatingNodePlot = !1, _) {
        u("[CreatingNodePlot]action be abort!");
        return;
      }
      const y = ht(p), M = y.clientX - l.x + 10, N = y.clientY - l.y + 10;
      if (o && o.disableClickCreate && Math.abs(h - M) < 5 && Math.abs(d - N) < 5) {
    ...
    setTimeout(() => {
      this.$dom.addEventListener("click", m), this.$dom.addEventListener("contextmenu", c);
    ...
    if (Y(t)) {
      ...
      o = (a, l, r, h, d) => {
        ...
        if (f && f.length > 1) {
          d.preventDefault();
          const m = f[1];
          n || (n = { x: m.clientX, y: m.clientY }, s = this.options.canvasZoom);
          const p = { x: c.clientX, y: c.clientY }, _ = { x: m.clientX, y: m.clientY }, y = Math.hypot(n.x - h.x, n.y - h.y), N = Math.hypot(_.x - p.x, _.y - p.y) / y, L = s * N;
          this.setZoom(L), this._dataUpdated();
        } else {
          const m = c.clientX, p = c.clientY;
          let _ = r.x + (m - h.x), y = r.y + (p - h.y);
          this.options.useHorizontalView ? (_ = r.x + (p - h.y), y = r.y - (m - h.x), this.setCanvasOffset(_, y)) : this.setCanvasOffset(_, y), this._dataUpdated();
        }
      };
    } else
      o = (n, s, a, l, r) => {
...
function F(e, i, t, o, n, s, a, l) {
  ...
  if (a ? (h = function(c) {
    ...
    if (r.functional) {
      r._injectStyles = h;
      var d = r.render;
      r.render = function(m, p) {
        return h.call(p), d(m, p);
      };
    } else {
...
const no = /* @__PURE__ */ function() {
  ...
  name: "SeeksRGLink",
  components: { RGLineSmart: Jt, RGLineTextByPath: no },
  props: {
    linkProps: {
      mustUseProp: !0,
      default: () => ({}),
...
const xo = /* @__PURE__ */ function() {
  ...
  components: { RGLineChecked: xo, SeeksRGNode: yt, SeeksRGLink: qt, SeeksRGLinePath: Qt, RGLineSmart: Jt },
  data() {
    return {};
  },
...
const So = /* @__PURE__ */ function() {
  ...
  components: { RelationGraphSingleGraph: So, SeeksRGNode: yt, SeeksRGLink: qt, SeeksRGLinePath: Qt },
  data() {
    return {};
  },
  inject: ["graph"],
...
const mn = /* @__PURE__ */ function() {
  ...
  components: { GraphLoading: mn, GraphOperateStuff: cn, GraphMiniToolBar: Jo, GraphToolBar: on, GraphMiniView: Vo, RGCanvas: zo, GraphDebugPanel: To },
  props: {
    options: {
      mustUseProp: !1,
      default: () => ({}),
  ...
  created() {
    window && (window.relationGraphDebug = this.options.debug), u("---------------------------graph created---------------------------", this), console.log(
      `%c relation-graph %c Version v${Vt} %c More info: https://github.com/seeksdream/relation-graph %c`,
    ...
    Z.version.slice(0, 4) === "2.5." && (e = !0), Z.version.slice(0, 4) === "2.6." && Number.parseInt(Z.version.split(".")[2]) <= 12 && (e = !0), e && (this.oldVueVersion = !0, console.error(`\u60A8\u7684Vue\u7248\u672C\uFF1A${Z.version}\u6CE8\u610F\uFF1A\u5F53\u4F60\u4F7F\u7528\u7684vue\u7248\u672C\u7B49\u4E8E\u4F4E\u4E8E2.6.12\u65F6\uFF0C\u56FE\u8C31\u4F1A\u663E\u793A\u4E0D\u6B63\u5E38\uFF0C\u53C2\u8003\u8FD9\u4E2A\u8FDE\u63A5\u89E3\u51B3\u8FD9\u4E2A\u95EE\u9898\uFF1Ahttps://github.com/seeksdream/relation-graph/issues/113`)), K || console.error("[relation-graph]Please introduce component screenfull, for example:https://cdnjs.cloudflare.com/ajax/libs/screenfull.js/5.1.0/screenfull.min.js"), Wt || console.error("[relation-graph]Please introduce component html2canvas, for example:https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js");
  },
  mounted() {
    u("---------------------------graph mounted---------------------------");
    const e = this.relationGraphCore ? new this.relationGraphCore(this.options, Rt(this)) : new Zt(this.options, Rt(this));
